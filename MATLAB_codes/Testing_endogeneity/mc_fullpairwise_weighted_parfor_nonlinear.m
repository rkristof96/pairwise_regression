...
% parameter sets
 T_list = [5000];%[50, 500, 1000, 5000];, 5000
...
correction_matrix = zeros(length(sigma_list),1);
...
dz0 = zeros(number_of_betas,1);
dz_star = zeros(number_of_betas,1);
dz_star_wave = zeros(number_of_betas,1);
    ...
    for i=(1:1:(T-1))
        for j=((i+1):1:T)
           % if i<j
                % calculate z values
                r_j = (y(i,r) + y(j,r))/2;
                dz0(counter,1) = logistic(y(j,r)) - logistic(y(i,r)) + omega(1,r)*var_y(1,r);
                dz_star(counter,1) = exp(-r_j)*logistic(r_j).^2 *(x(j,1) - x(i,1));
                dz_star_wave(counter,1) = -logistic_prime_prime(r_j)*(x(j,1) - x(i,1));
                %%%%%%%%%%%%%%%%%%%%
                % estimate delta 2 %
                %%%%%%%%%%%%%%%%%%%%
...
%---------------------------------------------------------------------------------------------------
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Bias correction - estimate eta %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
z_matr = [ones(number_of_betas,1), dz_star_wave(:,1)];  % this is matrix X in z_betahat = (X'X)^(-1)*(X'y)
z_sigma = std(dz_star_wave(:,1));
z_var_true = (z_sigma^2)*inv(z_matr'*z_matr);  % true variance-covariance matrix
mu_z(1,r) = mean(dz_star_wave(:,1));

% coefficients 
z_b_hat_all = zeros(2,reps);  % sFtore estimated betahats, r-th repetition in r-th column

delta_eta = zeros(1,reps);

%repetition starts
% s = 1;
%while s < reps+0.5
    z_avg     = mean(dz_star_wave(:,1));
    z0_avg     = mean(dz0);
    z0_avg_r   = z0_avg%;(r);
    numerator = 0;
    denominator = 0;
    for i=(1:1:T)
        z_dev = dz_star_wave(i,1)-z_avg;
        z0_dev = dz0(i,1)-z0_avg_r;
        numerator = numerator + z_dev*z0_dev;
        denominator = denominator + z_dev*z_dev;
    end
    z_b_hat              = numerator/denominator;
    z_b_hat_all(2,r)     = z_b_hat;
    z_b_hat_all(1,r)     = z0_avg_r - z_b_hat*z_avg;
    
    eta_hat              = dz0(:,1) - z_b_hat_all(1,r) - z_b_hat_all(2,r)*dz_star_wave(:,1);
    delta_eta(1,r) = sum(eta_hat);    
   %-----------------------------------------------------------------------------------------------
   % estimate beta 2
   % Obtain the delta-x weighted average of pairwise betas
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % sign of the correction term %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    correction(1,r) = 1/(number_of_betas * mu_z(1,r))*delta_eta(1,r);
    delta2 = b_hat_all(2,r) + 1/(number_of_betas * mu_z(1,r))*delta_eta(1,r);
    d_beta1_plus =  1/number_of_betas*sum(sum((dz0(:,1) - delta2^0.5.*dz_star(:,1) -  delta2*dz_star_wave(:,1)).^2));
    d_beta1_minus =  1/number_of_betas*sum(sum((dz0(:,1) + delta2^0.5.*dz_star(:,1) -  delta2*dz_star_wave(:,1)).^2));
    
    if d_beta1_plus > d_beta1_minus 
        correction(1,r) = (-1)*correction(1,r);
    end
...
%correction = 1./(mu_z*number_of_betas).*delta_eta;

%b_hat_all(2,:) = b_hat_all(2,:) + correction;
...
correction_matrix(sigma_ind,2) = nanmean(correction);
...
if strcmp(abs_dummy,'True')
    if strcmp(dist,'norm')
        ...
        correction_title = strcat('Results/Nonlinear_Norm_correction_Abs_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
        ...
        correction_title = strcat('Results/Nonlinear_Unif_correction_Abs_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
    ...
    if strcmp(dist,'norm')
        ...
        correction_title = strcat('Results/Nonlinear_Norm_correction_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
        ...
        correction_title = strcat('Results/Nonlinear_Unif_correction_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
...
parsave(correction_title, correction_matrix)
