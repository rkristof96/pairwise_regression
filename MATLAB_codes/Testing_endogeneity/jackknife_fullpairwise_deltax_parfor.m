...
reps = 1; % number of Monte Carlo repetitions
...
while rr < reps+0.5
    ...
    for i=(1:1:T-1)
        for j=((i+1):1:T)
                ...
                % calculate betahat
                x_avg     = (x(i,1)+x(j,1))/2;
                y_avg     = (y(i,rr)+y(j,rr))/2;
                numerator = y(j,rr) - y(i,rr);
    ...
    b_hat_all(1,rr)        = weighted_average_parwise_betas(1);
    b_hat_all(2,rr)        = weighted_average_parwise_betas(2);
    
    u_hat              = y(:,rr) - b_hat_all(1,rr) - b_hat_all(2,rr)*x;
    x_u_corr_all(1,rr)  = corr(x, u_hat);
    
    % test stat
    delta_u = delta_y - b_hat_all(2,rr)*delta_x;
    test_stat(sigma_ind, rr) = (sum(delta_x.*(delta_x*(beta-b_hat_all(2,rr))+delta_u))/(T_list(sample_size_ind)*(T_list(sample_size_ind)-1))*2);
...
if strcmp(abs_dummy,'True')
    if strcmp(dist,'norm')
        beta_title = strcat('Results/Jack_knife/Jackknife_Norm_beta_Abs_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
        corr_title = strcat('Results/Jack_knife/Jackknife_Norm_corr_Abs_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
        tstat_title = strcat('Results/Jack_knife/Jackknife_Norm_tstat_Abs_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
    else
        beta_title = strcat('Results/Jack_knife/Jackknife_Unif_beta_Abs_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
        corr_title = strcat('Results/Jack_knife/Jackknife_Unif_corr_Abs_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
        tstat_title = strcat('Results/Jack_knife/Jackknife_Unif_tstat_Abs_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');  
    end 
else
    if strcmp(dist,'norm')
        beta_title = strcat('Results/Jack_knife/Jackknife_Norm_beta_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
        corr_title = strcat('Results/Jack_knife/Jackknife_Norm_corr_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
        tstat_title = strcat('Results/Jack_knife/Jackknife_Norm_tstat_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
    else
        beta_title = strcat('Results/Jack_knife/Jackknife_Unif_beta_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
        corr_title = strcat('Results/Jack_knife/Jackknife_Unif_corr_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');
        tstat_title = strcat('Results/Jack_knife/Jackknife_Unif_tstat_deltax_T_',num2str(T_list(sample_size_ind)),'_',date_var,'.mat');  
    end
end
%Results/
