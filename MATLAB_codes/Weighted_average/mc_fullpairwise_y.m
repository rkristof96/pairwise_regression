...
while r < reps+0.5
    ...
    for i=(1:1:T-1)
        for j=(2:1:T)
            if i<j
                % add the earlier observed y
                first_y(1,counter) = y(i,r);
                % calculate betahat
                x_avg     = (x(i,1)+x(j,1))/2;
                y_avg     = (y(i,r)+y(j,r))/2;
    ...
    y_weight = first_y;
    weighting_y = y_weight;
    %weighting_y = 1./ y_weight;
    sum_y = sum(weighting_y);
    weighted_parwise_betas = pairwise_betas*weighting_y';
